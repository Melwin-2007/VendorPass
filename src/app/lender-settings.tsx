...
import { View, Text, StyleSheet, Pressable, ScrollView, Modal, Switch, Platform } from 'react-native';
...
export default function LenderSettingsScreen() {
  ...
  // Local state for instant UI update, synced to Supabase on change
  const [prefs, setPrefs] = useState({
    pref_loan_alerts: user?.pref_loan_alerts ?? true,
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colorBackground }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[styles.profileCard, { backgroundColor: theme.colorSurface, borderLeftColor: theme.colorPrimary, borderColor: theme.colorOutline, borderWidth: StyleSheet.hairlineWidth }]}>
          ...
          <View style={styles.profileInfo}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              <Text style={[styles.profileName, { color: theme.colorOnSurface }]}>{user?.name || 'Lender Name'}</Text>
              <View style={[styles.verifiedBadge, { backgroundColor: `${theme.success}15` }]}>
                <Text style={[styles.verifiedText, { color: theme.success }]}>Verified Lender</Text>
              </View>
            </View>
        ...
        {/* LENDING PREFERENCES */}
        <Text style={[styles.sectionTitle, { color: theme.textSecondary }]}>LENDING PREFERENCES</Text>
        <View style={[styles.cardGroup, { backgroundColor: theme.colorSurface, borderColor: theme.colorOutline, borderWidth: StyleSheet.hairlineWidth }]}>
          <SettingRow 
            icon="📍" 
            label="Lending Radius" 
            onPress={() => {}} 
            rightElement={<Text style={[styles.inlineValue, { color: theme.textSecondary }]}>5 km</Text>}
          />
          <SettingRow 
            icon="🎯" 
            label="Preferred Risk Tier" 
            onPress={() => {}} 
            rightElement={<Text style={[styles.inlineValue, { color: theme.textSecondary }]}>Gold & Above</Text>}
          />
          <SettingRow 
            icon="💰" 
            label="Auto-Approve Limit" 
            onPress={() => {}} 
            rightElement={<Text style={[styles.inlineValue, { color: theme.textSecondary }]}>₹25,000</Text>}
          />
          <SettingRow 
            icon="🔔" 
            label="Loan Request Alerts" 
            showDivider={false} 
            rightElement={<CustomToggle value={prefs.pref_loan_alerts} onValueChange={(v) => handleToggle('pref_loan_alerts', v)} />}
          />
        </View>
